...
class PendulumTorchDyn(AffineInControlTorchDyn):
    def f(self, state):
        ...
        return torch.stack([state[:, 1], 3 * env_config.g * torch.sin(state[:, 0]) / (2 * env_config.l)], dim=1)

    def g(self, state):
        if state.ndim == 1:
            return torch.tensor([0.0, 3/(env_config.m * env_config.l**2)])
        return torch.tensor([0.0, 3 / (env_config.m * env_config.l ** 2)]).repeat(state.size(0), 1)

    def dynamics_flat_return(self, state, u):
        res = self.dynamics(state, u)
        if state.ndim == 1:
            return res
        return res.flatten()
