...
config = {
    'init': {
        ...
        'wandb_project_name': 'multi_mass_dashpot',
        'save_models': True,
        'save_buffer': True,
        'step_save_freq': 1,
        'load_models': False,
...
env_config = AttrDict(
    ...
    timestep=0.01,
    max_u=[20.0, 20.0],
    max_u_for_safe_set=[20.0, 20.0],
    max_speed=[np.inf, np.inf, np.inf],
    max_speed_for_safe_set_training=[0.1, 0.1, 0.1],
    max_x=[40.0, 40.0, 40.0],
    # max_x_for_safe_set=12,      # not used in safeset from prop
    # min_x_for_safe_set=-4.0,    # not used in safeset from prop
    max_x_safe=[np.inf, np.inf, 5.0],
    min_x_safe=[-np.inf, -np.inf, -5.0],
    max_x_for_safe_set_training=[0.1, 0.1, 0.1],
    # out_width_up=0.4,           # not used in safeset from prop
    # out_width_down=0.4,         # not used in safeset from prop
    m=[1.0, 1.0, 1.0],
    k=[1.0, 1.0, 1.0],
    c=[1.0, 1.0, 1.0],
    omega=0.5,
    command_amplitude=2.0,
    max_T_for_safe_set=300,
    fixed_reset=False
)